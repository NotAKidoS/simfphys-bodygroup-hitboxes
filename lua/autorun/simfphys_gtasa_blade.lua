local V = {
	...
	Members = {
		...
		OnTick = function(ent)
			if ent.horn then
				ent.horn:ChangePitch( 120, 0 )
			end
			if (( ent:GetThrottle() < 0.1 ) && (ent:GetRPM() > 3500) && !ent.throttledoff) then
				ent.throttledoff = true
				ent:EmitSound( "gtasa/vehicles/39-40_throttleoff.wav" )
			end
			if ( ent:GetThrottle() > 0 ) then
				ent.throttledoff = false
			end
		end,
...
local light_table = {
	...
	Headlamp_sprites = {
		{pos = Vector(90.48,25.95,-5),size = 100,material = "sprites/light_ignorez"},
		{pos = Vector(90.48,-25.95,-5),size = 100,material = "sprites/light_ignorez"},
	},
