...
local function SpawnGibExploded(self,skin,Col,PxyClr,HBInfo)
    for id in SortedPairs(HBInfo) do
        ...
        if (HBInfo[id].GibModel) then
			...
			if ProxyColor then TheExtraGibs:SetProxyColor(PxyClr) end
            TheExtraGibs:Spawn()
...
local function SpawnGib(self, GibModel, GibOffset)
    if (GibModel) then
		...
		if ProxyColor then TheGib:SetProxyColor(self:GetProxyColor()) end
        TheGib:Spawn()
        TheGib:Activate()
...
local function OverrideExplode(self)
	self.ExplodeVehicle = function(self)
		...
		if self.GibModels then
			...
			bprop:SetColor( Col )
			bprop:SetSkin( skin )
			if ProxyColor then bprop:SetProxyColor(PxyClr) end
			...
			if self.CustomWheels == true and not self.NoWheelGibs then
				for i = 1, table.Count( self.GhostWheels ) do
					...
					if IsValid(Wheel) then
						...
						prop:SetOwner( bprop )
						if ProxyColor then prop:SetProxyColor(Wheel:GetProxyColor()) end
						prop:Spawn()
						prop:Activate()
        ...
        if self.NAKHitboxes then SpawnGibExploded(self,skin,Col,PxyClr,HBInfo) end
